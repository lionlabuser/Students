function plotconnectogramroi(fileorderconnectogram,MAT,label,plotLst)
...
%         end
% %       for izone = 1:numel(idlabelall); % nombre de canal dans chaque zone
%             idzone = strmatch({labelzone} ,idlabelall, 'exact'); %trouver le canal dans la liste
%             idlisti = [idlisti, idzone]; %creer une liste de la position de chaque canal dans la liste
% %       end
%         if numel(x)>1
%             x = x(1);
...
%             idch = chMAT{izone}; %position de chaque canal dans la liste
            idlist = [idlist, izone]; %liste de la position de chaque canal dans la liste
            idzone =[idzone,izone];
