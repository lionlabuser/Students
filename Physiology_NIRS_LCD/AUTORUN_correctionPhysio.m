...
% ------------ parameters to adjust for Phys_PCA & Phys_Prewhitening
jobP.ifFailReplaceWith = 0; % value to replace NaNs with if interpolation fails 1 is a good value for raw data; 0 otherwise
jobP.runPW = 0; %run prewhitening or not
jobP.type='linear';
jobP.ncomp = 1;
jobP.splittypes = true;
jobP.keep_components = false;
jobP.tune = 5; % number of standard deviations to define an outlier
jobP.modelorder='4xFs';
if jobP.runPW
    jobP.e_NIRSmatdirnewbranch = 'PCAPW'; %name of the new branch to create
else
    jobP.e_NIRSmatdirnewbranch = 'PCA'; %name of the new branch to create
end
...
for isubject=2:size(raw,1)
    ...
    if raw{isubject,idoit} %afaire
        ...
        if Phys_PCA ==1
            jobP.NIRSmat = NIRSmat; %modify
            jobP.ID = raw{isubject,1};
            nirs_run_physio_PCA(jobP); %run script

            %%create new branch
            jobP.m_newbranchcomponent = 1; %1 if copy paste SelectedFactors.mat and CorrectionApply.mat
            jobP2 = nirs_run_NIRSmatcreatenewbranch(jobP); %run script (create new folder + copy NIRS, SelectedFactors)
             dir1 = fileparts(jobP.NIRSmat{1}); %move saved graphs in the newbranch folder
             dir2 = fileparts(jobP2.NIRSmat{1});
             flist = dir(fullfile(dir1));
             tf = contains({flist.name},'Physio');
             for n = 1:nnz(tf)
                 idx = find(tf);
            ...
            %overwrite nirs data with the corrected data
            jobP2.globalmethod = jobP.e_NIRSmatdirnewbranch; %label that will be searched into the SelectedFactors.mat PARCOMP variable
            jobP2.DelPreviousData = 0;
            nirs_writeNIR_aftercorr(jobP2);%run script, write hdr & mrk
