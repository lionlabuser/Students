...
%___________________________________________________________________
function out = nirs_run_GLM_regressAUX_concat(job)
% 1) Load nirs.mat
...
for f = 1:size(rDtp,1)
    ...
    if all(isnan(nirsdata))%if all channels are bad after the YELLOW was took into consideration,
    ...
    %concatenate
    CONCATinfo(f,1)=size(CONCATnirsdata,1)+1;
    CONCATnirsdata=[CONCATnirsdata ; nirsdata];
CONCATcovdata=[ CONCATcovdata ; cov.data];
CONCATinfo(f,2)=size(CONCATnirsdata,1);
 ...
 clear tmp* cov.data goodCH nirsdata idbad
    ...
    for CH = 1:NC
        try
            [tmp.b,tmp.bint,~,~,tmp.stats] = regress(CONCATnirsdata(:,CH),CONCATcovdata);
            tmpbeta(:,CH) = tmp.b;
    ...
    tmpXcorr=CONCATnirsdata;
    for cc=1:length(cov.labels)
        tmpXm{cc} = tmpbeta(cc,:).* CONCATcovdata(:,cc);
    ...
    %CONCATinfo(f,2)=size(CONCATnirsdata,1);
for f = 1:size(CONCATinfo,1)
    if CONCATinfo(f,1) %for each valid block
    ...
    PARCOMP(Prow).AUX.data= CONCATcovdata(CONCATinfo(f,1):CONCATinfo(f,2),:);
    for cc=1:length(cov.labels)
    PARCOMP(Prow).AUX.indXm=tmpXm{cc}(CONCATinfo(f,1):CONCATinfo(f,2),:);
    end
    ...
    PARCOMP(Prow).data = CONCATnirsdata(CONCATinfo(f,1):CONCATinfo(f,2),:);
    PARCOMP(Prow).Xm = CONCATnirsdata(CONCATinfo(f,1):CONCATinfo(f,2),:)-tmpXcorr(CONCATinfo(f,1):CONCATinfo(f,2),:);
    PARCOMP(Prow).dataCORR = tmpXcorr(CONCATinfo(f,1):CONCATinfo(f,2),:);
